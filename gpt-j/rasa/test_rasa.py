...
agent = Agent.load(model_path='./rasa/models/model1.tar.gz')

result = asyncio.run(agent.parse_message(message_data='개나리 시 써봐'))
print(result)
...
print(result)
print(result['intent']['name'])
result = asyncio.run(agent.parse_message(message_data='꽃피는 봄을 소재로 시 한수 써줘.'))
print(result)
print(result['intent']['name'])
