...
def convert_multi(model_id: str, folder: str) -> List["CommitOperationAdd"]:
    filename = os.path.join(model_id, "pytorch_model.bin.index.json")
...
def convert_single(model_id: str, folder: str) -> List["CommitOperationAdd"]:
    pt_filename = os.path.join(model_id, "pytorch_model.bin")
...
def convert(model_id: str, force: bool = False) -> Optional["CommitInfo"]:
    ...
    from os import walk
    filenames = next(walk(model_id), (None, None, []))[2]  # [] if no file
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--model_id",
        type=str,
        default="/home/chang/AI/llm/t5tests/gpt-j/Models/OPT-2.7B-Erebus-instruct-native",
