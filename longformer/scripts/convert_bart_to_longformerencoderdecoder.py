...
from transformers import BartForConditionalGeneration, LEDForConditionalGeneration, LEDConfig
...
def create_long_model(
    ...
    config = LEDConfig.from_pretrained(base_model)
    model.config = config
...
def main():
    ...
    model = LEDForConditionalGeneration.from_pretrained(args.save_model_to, ignore_mismatched_sizes=True)
    ...
    decoder_input_ids = shift_tokens_right(input_ids[:, :5], tokenizer.pad_token_id, 2)
